...
//		se ta na reta final e vai para outra casa da reta final (nao tirou o numero exato para chegar no final)
		else if (p.get_currentCasa().is_reta_final() && this.is_reta_final())
			return false;
...
//		se tem 1 ou 0 pecas, se tem mais de 2 (casa inicial e final), 
	...
	protected boolean is_barreira() {
		if (is_casa_inicial() || is_casa_de_saida() || is_abrigo())
			return false;
		if (num_pecas < 2)
			return false;
		else if (peca_arr[0].get_cor() == peca_arr[1].get_cor())
			return true;
		return false;
	}
